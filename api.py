...
def load_audio():
    ...
    if lang not in conf.SAMPLE_WAV or model not in conf.WHISPER["model"]["2"]:
        abort(400)
    try:
    ...
    with os.popen(cmd % (auid, fext(wav))) as pipe:
        if os.wait()[1] == 0:
            ...
            status = {"error": "[FFMPEG] %s" % pipe.read()}
    os.remove(auid)
    if connector is not None:
        connector.close()
    return status
...
def get_status_or_result(auid=None):
    ...
    if data[2]:
        return {"auid": auid, "status": "failed", "log": data[3]}
    if data[1]:
...
# (echo -n '{"lang": "ru", "model": "lev2", "data": "'; base64 inp.mp3; echo '"}') | curl -X POST -H "Content-Type: application/json" -d @- http://127.0.0.1:5000/
